...
<script type="text/javascript">
	...
	function onSubmiForm(){
		...
		var eclassTraffic = document.getElementsByName("classTraffic");
		...
		//classTraffic(checkbox)
		for(var i=0; i<eclassTraffic.length; i++){
			if(eclassTraffic[i].checked){
				classTraffic += "classTraffic=" + eclassTraffic[i].value + "&";
			}
		}
		...
		if(interfacesId=="" || classTraffic=="" || margeAgregation=="" || niveauAgregation=="" || startDate=="" || endDate==""){
			alert("Alert: Vueillez remplir tous les champs");
			
		}else{
			link += interfacesId + classTraffic + grapheType + grapheTypeChart + margeAgregation + niveauAgregation + startDate + endDate;
			myPopupGraph(link);
		}
		}catch(e){
			console.log(e);
	...
	<div class="container_12">
		...
		<div class='<s:property value="gridClass" />'>
			<div class="box round first">
				<h2>Générer le graphe de trafics par classes</h2>
				<div class="block">
				
					<!-- Message Error & Info & warning -->
					<ul class="message warning">
						<li>
							NB: Notez-vous que vous pouvez choisir plusieurs interfaces, maximum <strong>3</strong> interfaces. (<strong>CTRL + Clique</strong> pour selectionner plusieurs interfaces).
						</li>
					</ul>
					...
					<form action='<s:property value="rootUrl"/>traffics/perClassGraphe' method="get" onsubmit="return onSubmiForm();">
							<tr>
							...
							<tr>
								<td>Classe:</td>
								<td>
									<s:checkbox name="classTraffic" fieldValue="0" checked="checked" />0
									<s:checkbox name="classTraffic" fieldValue="1" />1
									<s:checkbox name="classTraffic" fieldValue="2" />2
									<s:checkbox name="classTraffic" fieldValue="3" />3
									<s:checkbox name="classTraffic" fieldValue="4" />4
									<s:checkbox name="classTraffic" fieldValue="5" />5
									<s:checkbox name="classTraffic" fieldValue="6" />6
									<s:checkbox name="classTraffic" fieldValue="7" />7
								</td>
							</tr>
